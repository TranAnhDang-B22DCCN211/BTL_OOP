...
import com.techstore.exception.InputDataException;
...
public class SanPhamController {
    ...
    private void initController() {
        ...
        view.getItemSua().addActionListener(e -> suaSanPhamDuocChon());
        view.getItemXoa().addActionListener(e -> xoaSanPhamDuocChon());
        view.getBtnQuayLai().addActionListener(e -> quayLaiMenu());
    }

    private void xuLyClickBang(MouseEvent e) {
    ...
    private void suaSanPhamDuocChon() {
        ...
        SanPhamDialog dialog = taoDialogSuaSanPham(sp);
        dialog.getBtnLuu().addActionListener(event -> luuSanPhamDaSua(dialog, sp));
        dialog.setVisible(true);
    }

    private SanPhamDialog taoDialogSuaSanPham(SanPham sp) {
        SanPhamDialog dialog = new SanPhamDialog(view, "Sửa Thông Tin Sản Phẩm");
        dialog.getTxtMaSp().setText(sp.getMaSanPham());
        dialog.getTxtMaSp().setEditable(false);
        ...
        dialog.getCboLoaiSp().setEnabled(false);
        return dialog;
    }

    private void luuSanPhamDaSua(SanPhamDialog dialog, SanPham sp) {
        try {
            ...
            double giaBan = parseGiaBan(giaBanStr);
            int soLuong = parseSoLuong(soLuongStr);
    ...
    private double parseGiaBan(String value) {
        try {
            return Double.parseDouble(value);
        } catch (NumberFormatException ex) {
            throw new InputDataException("Giá bán phải là số hợp lệ!", ex);
        }
    }
    ...
    private int parseSoLuong(String value) {
        try {
            ...
            throw new InputDataException("Số lượng phải là số nguyên hợp lệ!", ex);
        }
    }
}
