...
import com.techstore.exception.HoaDonException;
...
public class HoaDonService {
    ...
    public boolean taoHoaDon(HoaDon hd) throws HoaDonException {
        if (hd.getMaHoaDon() == null || hd.getMaHoaDon().trim().isEmpty()) {
            throw new HoaDonException("Mã hóa đơn không được để trống!");
        }
        if (hd.getTenKhachHang() == null || hd.getTenKhachHang().trim().isEmpty()) {
            throw new HoaDonException("Tên khách hàng không được để trống!");
        }
        if (hd.getDanhSachChiTiet() == null || hd.getDanhSachChiTiet().isEmpty()) {
            throw new HoaDonException("Hóa đơn phải có ít nhất 1 sản phẩm!");
        }
        if (hoaDonRepo.findById(hd.getMaHoaDon()) != null) {
            throw new HoaDonException("Mã hóa đơn đã tồn tại trong hệ thống!");
        }
    ...
    private void kiemTraTonKho(HoaDon hd) throws HoaDonException {
        ...
        for (ChiTietHoaDon ct : hd.getDanhSachChiTiet()) {
            if (ct.getMaSanPham() == null || ct.getMaSanPham().trim().isEmpty()) {
                throw new HoaDonException("Mã sản phẩm trong hóa đơn không hợp lệ!");
            }
            if (ct.getSoLuong() <= 0) {
                throw new HoaDonException("Số lượng mua phải lớn hơn 0!");
            }
        ...
        for (Map.Entry<String, Integer> entry : tongSoLuongTheoSanPham.entrySet()) {
            ...
            if (sp == null) {
                throw new HoaDonException("Không tìm thấy sản phẩm: " + entry.getKey());
            }
            if (sp.getSoLuongTon() < entry.getValue()) {
                throw new HoaDonException("Không đủ tồn kho cho sản phẩm "
    ...
    public boolean xoaHoaDon(String maHd) throws HoaDonException {
        if (maHd == null || maHd.trim().isEmpty()) {
            throw new HoaDonException("Mã hóa đơn không hợp lệ!");
        }
